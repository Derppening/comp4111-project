...
public class BooksPutDataAccess extends Book {
    ...
    public static int updateBook(@NotNull Connection con, long id, boolean available) {
        final var book = getBook(con, id);
        if (book.isEmpty()) {
            return 2;
        } else if (book.get(0).isAvailable() == available) {
            return 1;
        } else {
            try (var stmt = con.prepareStatement("UPDATE Book SET available = ? WHERE id = ?")) {
                stmt.setBoolean(1, available);
                stmt.setLong(2, id);
                return stmt.executeUpdate() > 0 ? 0 : 1;
            } catch (SQLException e) {
    ...
    public static List<Book> getBook(Long id) {
        ...
        try {
            ...
            params.add(id);
            final var bookInDb = QueryUtils.queryTable(null, "Book", "where id = ?", params, Book::toBook);
